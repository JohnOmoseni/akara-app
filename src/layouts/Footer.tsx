...
const Footer = ({ isVisible }: { isVisible: boolean }) => {
	...
	return (
		<footer
			className={cn(
				"bg-background w-full mx-auto z-[200] border fixed sm:bottom-3 inset-0 md:left-[80px] top-auto sm:max-w-[350px] sm:rounded-lg sm:drop-shadow-[0_1px_4px_rgb(0_0_0_/_0.08)] max-sm:border-t border-border-100",
			...
			<div className="flex flex-row justify-around gap-4 px-4 pt-0.5 max-sm:max-h-[60px] min-h-14">
				{footerTabs.map(({ icon: Icon, label, href, value }, index) => {
					...
					return (
						<div
							className={cn(
								"relative px-5 py-2 sm:pt-3.5 flex-column items-center",
								isActive
