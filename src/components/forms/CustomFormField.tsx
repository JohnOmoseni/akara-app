					...
					{field?.type === "password" && (
						...
						>
							{showPassword ? (
								...
								<EyeOff size={20} className="text-foreground-variant" />
							)}
						</span>
					)}
				</>
		...
		case FormFieldType.SELECT:
			return (
				...
				>
					...
					<SelectContent
						className={cn("shad-select-content z-[999]", selectContainerStyles)}
					>
						{props.children}
...
const CustomFormField = (props: CustomProps) => {
	...
	const result = ![FormFieldType.SKELETON, FormFieldType.CHECKBOX].includes(
		fieldType
	) ? (
		<>
			{label && (
				<Label className={cn("ml-1 inline-flex", labelStyles)}>{label}</Label>
			)}

			<div
				className={cn(
					"row-flex-start relative w-full gap-0.5 mt-1.5 overflow-hidden rounded-lg border border-border bg-background shadow-sm",
					containerStyles,
